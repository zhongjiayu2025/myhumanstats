...
const DeadPixelPage: React.FC = () => {
  ...
  // Listen for ESC to exit UI mode handling
  useEffect(() => {
    const handleChange = () => {
      if (!document.fullscreenElement) {
        setIsFullscreen(false);
    ...
    >
      {!isFullscreen && (
        <div className="animate-in fade-in">
          <SEO 
            title="Dead Pixel Test - Check Monitor & Screen"
            description="Free online Dead Pixel Checker. Full-screen color cycling (Red, Green, Blue, White, Black) to detect stuck or dead pixels on LCD/OLED screens."
          />
          ...
          <div className="mt-12 border-t border-zinc-800 pt-8">
             <h3 className="text-lg font-bold text-white mb-4">Hardware looks good? Test your bio-ware.</h3>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <a href="/test/color-blind-test" className="p-4 bg-zinc-900/50 border border-zinc-800 hover:border-primary-500/50 transition-colors rounded block group">
                   <div className="text-primary-400 font-bold mb-1 group-hover:underline">Color Blind Test</div>
                   <p className="text-xs text-zinc-500">Ensure your eyes are calibrated, not just your screen.</p>
                </a>
                <a href="/test/contrast-test" className="p-4 bg-zinc-900/50 border border-zinc-800 hover:border-primary-500/50 transition-colors rounded block group">
                   <div className="text-primary-400 font-bold mb-1 group-hover:underline">Contrast Sensitivity</div>
                   <p className="text-xs text-zinc-500">Can you distinguish subtle shade differences?</p>
                </a>
             </div>
          </div>
        </div>
