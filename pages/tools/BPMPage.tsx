...
import { Activity, RotateCcw, Clock, Music } from 'lucide-react';
...
const BPMPage: React.FC = () => {
  ...
  const handleTap = () => {
     ...
     setTaps(prev => {
        // Reset if pause is too long (> 2.5s)
        if (prev.length > 0 && now - prev[prev.length - 1] > 2500) {
           return [now];
        }
        // Keep last 10 taps for rolling average
  ...
  return (
    <div className="max-w-4xl mx-auto py-12 px-4 animate-in fade-in">
       <SEO 
          title="BPM Counter - Tap Beats Per Minute"
          description="Free online BPM Tapper. Calculate the tempo of any song by tapping along. Essential tool for DJs, musicians, and producers."
       />
          ...
          <div className="lg:col-span-5 space-y-8">
             ...
             <div className="bg-zinc-900/30 border border-zinc-800 p-6 rounded-xl">
                ...
                <div className="space-y-3">
                   <a href="/test/rhythm-test" className="block text-sm text-zinc-400 hover:text-primary-400 underline decoration-zinc-700 underline-offset-4">Rhythm Test (Stability Check)</a>
                   <a href="/test/cps-test" className="block text-sm text-zinc-400 hover:text-primary-400 underline decoration-zinc-700 underline-offset-4">Clicks Per Second Test</a>
                </div>
             </div>
          </div>
