...
import { Mic, Square, Play, Volume2, AlertCircle, RefreshCw, Settings2 } from 'lucide-react';
...
const MicTestPage: React.FC = () => {
  ...
  const drawVisualizer = () => {
     ...
     const draw = () => {
        ...
        // Clear based on CSS width/height logic which setupHiDPICanvas handles internally via scale
        // But clearRect uses logical pixels
        ...
        for(let i = 0; i < bufferLength; i++) {
           ...
           if (barHeight > 80) ctx.fillStyle = '#22d3ee'; // Cyan peak
    ...
    <div className="max-w-4xl mx-auto py-12 px-4 animate-in fade-in">
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          ...
          <div>
             ...
             {error && (
                <div className="bg-red-900/10 border border-red-500/50 p-6 rounded-lg mb-6">
                   <div className="flex items-start gap-3 mb-4">
                       <AlertCircle className="text-red-500 shrink-0 mt-1" size={24} />
                       <div>
                           <h3 className="font-bold text-red-400 text-sm uppercase">Access Denied</h3>
                           <p className="text-sm text-red-200 mt-1">Browser blocked microphone access.</p>
                       </div>
                   </div>
                   <div className="bg-black/30 p-3 rounded text-xs text-zinc-400 font-mono mb-3">
                       <Settings2 size={12} className="inline mr-1"/>
                       <strong>How to fix:</strong> Click the lock icon 🔒 next to the URL bar, enable "Microphone", then refresh.
                   </div>
                   <button onClick={() => window.location.reload()} className="btn-secondary text-xs w-full">Refresh Page</button>
                </div>
             )}
